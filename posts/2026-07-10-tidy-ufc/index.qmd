...
# Create the text that will appear when hovering over a point in the ggiraph object
tooltip <- paste0(
...
# All champs on 329 are men, so get only Men's PFP rankings from the rankings data
pfp <- ufc_rankings_dataset |>
  filter(weightclass == "Pound-for-Pound" | weightclass == "Men's Pound-for-Pound")

# PFP split into separate Men's and Women's lists after 2020, so I'm consolidating the two labels like so
...
# Load in timetk for easy date-based summarizing and aggregate the data at the monthly level
library(timetk)
